...
last_frame = 13 # MUST UPDATE THIS TO 100 LATER
...
data_BiMax0 = pg.GData('gk_lorentzian_mirror-ion_BiMaxwellianMoments_0.gkyl')
...
for i in range(last_frame):
  data_BiMax = pg.GData('gk_lorentzian_mirror-ion_BiMaxwellianMoments_'+str(i)+'.gkyl')
...
data_field0 = pg.GData('gk_lorentzian_mirror-field_0.gkyl')
...
for i in range(last_frame):
  data_field = pg.GData('gk_lorentzian_mirror-field_'+str(i)+'.gkyl')
...
plt.tight_layout()
# plt.show()
plt.savefig('bimaxwellian_moments_and_field_std_lorentzian.pdf')
plt.close()  # Close figure to free memory


# Plot to compare the initial conditions with the last frame

data_BiMax = pg.GData('gk_lorentzian_mirror-ion_BiMaxwellianMoments_0.gkyl')
interp_BiMax = pg.GInterpModal(data_BiMax, 1, 'ms')
x, dens = interp_BiMax.interpolate(0)
x, upar = interp_BiMax.interpolate(1)
x, Tpar_div_m = interp_BiMax.interpolate(2)
...
dens_0 = np.squeeze(dens)
upar_0 = np.squeeze(upar)/c_s
Tpar_div_m_0 = np.squeeze(Tpar_div_m)
Tperp_div_m_0 = np.squeeze(Tperp_div_m)

Tpar_0 = Tpar_div_m_0 * mi / eV  # Convert to eV
Tperp_0 = Tperp_div_m_0 * mi / eV  # Convert to eV

data_BiMax = pg.GData('gk_lorentzian_mirror-ion_BiMaxwellianMoments_'+str(last_frame)+'.gkyl')
interp_BiMax = pg.GInterpModal(data_BiMax, 1, 'ms')
x, dens = interp_BiMax.interpolate(0)
x, upar = interp_BiMax.interpolate(1)
x, Tpar_div_m = interp_BiMax.interpolate(2)
...
Tperp_div_m = np.squeeze(Tperp_div_m)
Tpar = Tpar_div_m * mi / eV  # Convert to eV
Tperp = Tperp_div_m * mi / eV  # Convert to eV

data_field = pg.GData('gk_lorentzian_mirror-field_0.gkyl')
interp_field = pg.GInterpModal(data_field, 1, 'ms')
x, phi = interp_field.interpolate(0)
phi_0 = np.squeeze(phi) * eV / Te0

data_field = pg.GData('gk_lorentzian_mirror-field_'+str(last_frame)+'.gkyl')
interp_field = pg.GInterpModal(data_field, 1, 'ms')
x, phi = interp_field.interpolate(0)
phi = np.squeeze(phi) * eV / Te0
...
x_norm = np.squeeze(nodes_Z)

# Plot density comparison
ax[0, 0].plot(x_norm, dens_0, label='Initial (t=0)', color="#0080D5", linewidth=2)
ax[0, 0].plot(x_norm, dens, label=f'Final (t={last_frame})', color="#D55E00", linewidth=2, linestyle='--')
ax[0, 0].axvline(x=0.98, color='black', linestyle=':', alpha=0.7, linewidth=1)
ax[0, 0].axvline(x=-0.98, color='black', linestyle=':', alpha=0.7, linewidth=1)
ax[0, 0].set_ylabel(r'$n_i$, (m$^{-3}$)')
# ax[0, 0].legend()
# ax[0, 0].grid(True)

# Plot parallel velocity comparison
ax[0, 1].plot(x_norm, upar_0, label='t=0', color="#0080D5", linewidth=2)
ax[0, 1].plot(x_norm, upar, label=f't={last_frame/100} ms', color="#D55E00", linewidth=2, linestyle='--')
ax[0, 1].axvline(x=0.98, color='black', linestyle=':', alpha=0.7, linewidth=1)
ax[0, 1].axvline(x=-0.98, color='black', linestyle=':', alpha=0.7, linewidth=1)
ax[0, 1].set_ylabel(r'$u_{||} / c_s$')
ax[0, 1].legend(frameon=False)
# ax[0, 1].grid(True)

# Plot parallel temperature comparison
ax[1, 0].plot(x_norm, Tpar_0, label='Initial (t=0)', color="#0080D5", linewidth=2)
ax[1, 0].plot(x_norm, Tpar, label=f'Final (t={last_frame})', color="#D55E00", linewidth=2, linestyle='--')
ax[1, 0].axvline(x=0.98, color='black', linestyle=':', alpha=0.7, linewidth=1)
ax[1, 0].axvline(x=-0.98, color='black', linestyle=':', alpha=0.7, linewidth=1)
...
# Plot perpendicular temperature comparison
ax[1, 1].plot(x_norm, Tperp_0, label='Initial (t=0)', color="#0080D5", linewidth=2)
ax[1, 1].plot(x_norm, Tperp, label=f'Final (t={last_frame})', color="#D55E00", linewidth=2, linestyle='--')
ax[1, 1].axvline(x=0.98, color='black', linestyle=':', alpha=0.7, linewidth=1)
ax[1, 1].axvline(x=-0.98, color='black', linestyle=':', alpha=0.7, linewidth=1)
...
ax[1, 2].plot(x_norm, phi_0, label='Initial (t=0)', color="#0080D5", linewidth=2)
ax[1, 2].plot(x_norm, phi, label=f'Final (t={last_frame})', color="#D55E00", linewidth=2, linestyle='--')
ax[1, 2].axvline(x=0.98, color='black', linestyle=':', alpha=0.7, linewidth=1)
ax[1, 2].axvline(x=-0.98, color='black', linestyle=':', alpha=0.7, linewidth=1)
ax[1, 2].set_ylabel(r'$e \phi / T_e$')
# ax[1, 2].set_xlabel(r'z, (m)')
# ax[1, 2].legend()
# ax[1, 2].grid(True)
...
plt.tight_layout()
plt.savefig('initial_vs_final_comparison_lorentzian.pdf')
plt.close()  # Close figure to free memory
