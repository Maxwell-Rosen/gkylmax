...
eval_density_ion_source(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  if (fabs(z) <= 0.98)
  {
    fout[0] = app->ion_source_amplitude * (1 - pow(fabs(z), 6)/0.98);
  }
  else
  {
    fout[0] = 1e-16;
...
create_ctx(void)
{
  ...
  // Source parameters
  double ion_source_amplitude = 1.85884e20; // Beam intM0 = 3.172138e+20
  double ion_source_temp = 18514.3 * eV; // Beam intM2 = 7.682495e+32
  ...
  for (int i=0; i<(num_phases-1)/2; i++) {
    ...
    if (i == 0) {
      poa_phases[2*i+1].duration = tau_first_fdp;
    }
    else {
