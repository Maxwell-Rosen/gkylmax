...
def psi_f(R, Z):
    ...
    # k = np.sqrt(k2)
    # Aphi += np.sum(-mu0*Im/np.pi * np.sqrt(a/R) * ((k2 - 2)/2/k*ellipk(k2) + ellipe(k2)/k))
    return Aphi * R
...
for i,Ri in enumerate(Rgrid):
    for j,Zj in enumerate(Zgrid):
        ...
        if Ri < 1e-17:
            Ri = Rgrid[1]
        BzRZ[i,j] = 1/Ri**2 * (psi_f(Ri+dR,Zj) - psi_f(Ri-dR,Zj))/(2*dR)
        BrRZ[i,j] = -1/Ri * (psi_f(Ri,Zj+dZ) - psi_f(Ri,Zj-dZ))/(2*dZ)
        BRZ[i,j] = np.sqrt(BrRZ[i,j]**2 + BzRZ[i,j]**2)

plt.figure()
contour = plt.contour(Rgrid,Zgrid, psiRZ.T, levels = np.linspace(0, 3e-8, 50))
