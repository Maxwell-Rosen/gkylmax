...
pgkyl --c2p-vel gk_lorentzian_mirror-ion_mapc2p_vel.gkyl gk_lorentzian_mirror-ion_${frame}.gkyl interp integ 2 ev 'f abs' pl --xlabel 'Axial Position z (m)' --ylabel 'Parallel Velocity $v_\parallel$ (m/s)' --logz --zmin 1e-35 --title "Ion Distribution Integrated over μ (Frame ${frame}) - WHAM1X" --saveas python-plots/ion_distf_integrated_mu_frame${frame}.png --no-show &
...
pgkyl gk_lorentzian_mirror-ion_$(${frame}-5).gkyl gk_lorentzian_mirror-ion_${frame}.gkyl interp ev 'f[1] f[0] - f[0] / abs' sel --z0 0.0 pl --title 'Fractional error between begining and end of ${frame}-5 and ${frame}' --saveas python-plots/ion_distf_diff_${frame}_$(${frame}-5).png --no-show &
