...
# Output grid for velocity 
u0 = x*v_norm
u02 = u0*u0

# Convert to real velocity grid with following
...
for ipsi in range(bsegment[iam],bsegment[iam]+lsegment[iam]):
    ...
    for ispec in range(N_spec):
        ...
        for il in range(1,zDim):
            ...
            f_tmp[ispec,ipsi,il,:,:] = remap_f(cql,ispec,ipsi,il,vbndry1,sin02thbndry1,vbndry2,vp2,zbnce,zbnce2,itype00)
        #out of il loop
    #out of ispec loop
...
if (iam == 0):
    ...
    hf.create_dataset('vGrid',data=v0)         #[cm/s]
    ...
    hf.create_dataset('f_dist',data=f_out)     #[cm^-6 s^3]
    hf.close()
