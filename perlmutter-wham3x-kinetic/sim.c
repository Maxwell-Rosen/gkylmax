...
read_ion_distf(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double t_norm_cord = (x_fa[2] + tmin) / (tmax - tmin);
...
create_ctx(void)
{
  ...
  int num_cell_vpar = 8; // 32
  int num_cell_mu = 8;  // 32
  int num_cell_z = 8;  //100
  int unif_z_cells = 16; //288 for 1d publish
  int num_cell_psi = 4; //16
  int num_cell_angle = 4; //16
  int poly_order = 1;
