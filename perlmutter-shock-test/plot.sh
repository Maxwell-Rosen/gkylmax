name="outputs/gk_bgk_periodic_sod_shock_1x2v_p1"
nonunifname="outputs/gk_bgk_periodic_sod_shock_1x2v_p1_nonunif"
sp="neut"
# frame="1"
for frame in {0..1}
do
  pgkyl "$name-$sp"_BiMaxwellianMoments_"$frame.gkyl" "$nonunifname-$sp"_BiMaxwellianMoments_"$frame.gkyl"\
    interp -b ms -p1\
    pl --title "frame $frame Moments" \
    --saveas "python-plots/BiMaxMoms-$frame.png" --no-show -f0 --no-legend&
done
