...
data_dir        = '.' #[ Where Gkeyll data is located.
simulation_name = 'gk_lorentzian_mirror' #[ Name of Gkeyll simulation.
species_names   = ['ion'] #[ Name of the particles species simulated.
...
if plot_relative_error:
  ...
  for sI in range(len(species_names)):
    ...
    print ("path = ",data_path + species + '_fdot_integrated_moms.gkyl')
    time_fdot, fdot_s = read_dyn_vector(data_path + species + '_fdot_integrated_moms.gkyl')
  ...
  hpl2a.append(ax2a.semilogy(time_fdot[0:-1], np.abs(mom_err_norm), color=defaultColors[0], linestyle=lineStyles[0], linewidth=2))
