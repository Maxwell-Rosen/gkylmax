...
in_folders_orgnaized = 1 # 1 if it's in the folders, 0 if data is in present directory
...
if plot_stitched_integrated_moms:
  ...
  time_fit = time[2000:] * 1e5
  M0_ion_fit = M0_ion[2000:] * 1e-19
  ...
  popt, pcov = curve_fit(exp_func, time_fit, M0_ion_fit, p0=[-1, 1e-3, 2])
  ...
  source = np.array([5.167229357643037, 8])

  total_M0 = np.array([5.8960673300757049, 6.4275217249160654])
