...
plot_mom_ic               = False   #[ Plot the initial moments.
...
outDir = '/home/mr1884/scratch/wham/postprocessing'
...
def setTickFontSize(axIn, fontSizeIn):
  for tick in axIn.xaxis.get_major_ticks():
    tick.label1.set_fontsize(fontSizeIn)
    tick.label2.set_fontsize(fontSizeIn)
  for tick in axIn.yaxis.get_major_ticks():
    tick.label1.set_fontsize(fontSizeIn)
    tick.label2.set_fontsize(fontSizeIn)
...
#    ax5[0].set_xlim( xIcc[0][iLo], xIcc[0][iUp-1] )
  ...
  ax5[1].set_ylabel(r'$e\phi(z=0)/T_{e0}$', fontsize=xyLabelFontSize)
...
if plot_nonuniformz:
  ...
  #[ Load the ion distribution function and its grid.
  dataDir  = '/home/mr1884/scratch/wham/gk2/'
  fileName = 'gk2-wham1x2v'    #.Root name of files to process.
  ...
  def z_xi(xi):
    # Argument xi: computational space uniform
    # Return z, the non-uniform location along the field line that the point is evaluated at
    delta_y = 1  # Total change in slope of the transformation
    y_max = 2  # Maximum slope of the transformation
    zMin = -2.515312
    zMax = 2.515312
    z_m = 0.982544
    a = (-delta_y / (-zMin - z_m) * (z_m ** 2 / 2 + zMin * z_m) \
      - y_max * z_m \
      + delta_y / (-zMin - z_m) * (-(zMin ** 2) / 2) \
      - y_max * zMin)
    b = (-delta_y / z_m) * z_m ** 2 / 2 + y_max * z_m
    c = (-delta_y / z_m) * z_m ** 2 / 2 + y_max * z_m
    d = (delta_y / (zMax - z_m) * (zMax ** 2 / 2 - zMax * zMax) \
      + y_max * zMax \
      - (delta_y / (zMax - z_m) * (z_m ** 2 / 2 - z_m * zMax)) \
      - y_max * z_m)
    z, unnormalized_z = None, None
    if isinstance(xi, (int, float)):
      if (zMin <= xi <= zMax):
        if (xi <= -z_m):
          z = (-delta_y / (-zMin - z_m) * (xi ** 2 / 2 - zMin * xi)
            + y_max * xi
            + delta_y / (-zMin - z_m) * (-(zMin ** 2) / 2)
            - y_max * zMin)
        elif (xi <= 0.0):
          z = (delta_y / z_m) * xi ** 2 / 2 + y_max * xi - delta_y / z_m * z_m ** 2 / 2 + y_max * z_m + a
        elif (xi <= z_m):
          z = (-delta_y / z_m) * xi ** 2 / 2 + y_max * xi + a + b
        else:
          z = (delta_y / (zMax - z_m) * (xi ** 2 / 2 - xi * zMax)
            + y_max * xi
            - (delta_y / (zMax - z_m) * (z_m ** 2 / 2 - z_m * zMax))
            - y_max * z_m + a + b + c)
        unnormalized_z = (z / (a + b + c + d)) * (zMax - zMin) + zMin
      else:
        unnormalized_z = xi
    else:
      unnormalized_z = []
      for i in range(len(xi)):
        if (zMin <= xi[i] <= zMax):
          if (xi[i] <= -z_m):
            z = (-delta_y / (-zMin - z_m) * (xi[i] ** 2 / 2 - zMin * xi[i])
              + y_max * xi[i]
              + delta_y / (-zMin - z_m) * (-(zMin ** 2) / 2)
              - y_max * zMin)
          elif (xi[i] <= 0.0):
            z = (delta_y / z_m) * xi[i] ** 2 / 2 + y_max * xi[i] - delta_y / z_m * z_m ** 2 / 2 + y_max * z_m + a
          elif (xi[i] <= z_m):
            z = (-delta_y / z_m) * xi[i] ** 2 / 2 + y_max * xi[i] + a + b
          else:
            z = (delta_y / (zMax - z_m) * (xi[i] ** 2 / 2 - xi[i] * zMax)
              + y_max * xi[i]
              - (delta_y / (zMax - z_m) * (z_m ** 2 / 2 - z_m * zMax))
              - y_max * z_m + a + b + c)
          unnormalized_z.append((z / (a + b + c + d)) * (zMax - zMin) + zMin)

        else:
          unnormalized_z.append(xi[i])
    return unnormalized_z
  
  #print(x_i.())
  # print(x_i[0])
  #print(z_xi(x_i[0]))
  print("Number of cells +1:", len(x_i[0]))
  print("Grid spacing if this was uniform density, dz = ",np.mean(np.diff(x_i[0])))
  ...
  hpl4a = list()
  hpl4a.append(ax4[0].plot(z_xi(xIntC_i[0]), bmag, color=defaultBlue, linestyle='--', marker='.',markersize=5))
  hpl4a.append(ax4[0].plot(z_xi(xIntC_i[0]), bmag*0, color=defaultBlue, linestyle='none', marker='.',markersize=1))
