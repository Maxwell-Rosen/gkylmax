...
resolution_list = np.array([32, 64, 96, 128])
...
for ir in range(len(resolution_list)):
    ...
    for iframe in range(len(frame_nums)):
      filename_read = dataDir + str(resolution) + '/BiMaxwellianMoments/' + \
        filename + '-elc_BiMaxwellianMoments_'+str(frame_nums[iframe])+'.gkyl'
      
      frame_data = pg.GData(filename_read)
      frame_interp = pg.GInterpModal(frame_data, polyOrder, 'ms')
      coords, ne = frame_interp.interpolate(0)
      midplane_ne = ne[int(ne.shape[0]/2)]
      densities[iframe] = midplane_ne

    # Fit a line to the data.
    time = time_per_frame * np.arange(len(frame_nums))
    def fit_function(t, a, b, c):
       return a * np.exp(b * t) + c
        # return m*t + b
    # Initial guess for the fit parameters.
    slope = (densities[-1] - densities[0])/(time[-1] - time[0])
    ...
    # plt.plot(time, densities, 'bo')
    # plt.plot(time, fit_function(time, *popt), 'r-')
    # plt.xlabel('Time')
    # plt.ylabel('Density')
    # plt.yscale('log')
    # plt.legend(['Data', 'Fit'])
