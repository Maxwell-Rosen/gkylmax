...
struct gk_mirror_ctx
{
  ...
  double tau_fdp_extra_standalone; // FDP extra duration for standalone runs in microseconds
  int extra_frames_fdp_extra_standalone; // Number of extra frames for FDP extra phase
...
create_ctx(void)
{
  ...
  double tau_oap = 0.0; // OAP duration in microseconds
  double tau_fdp = 0.0; // FDP duration in microseconds
  double tau_fdp_extra = 0; // Extra FDP duration in final cycle
  int frames_per_phase = 0; // Frames per phase
  int num_cycles = 0; // Number of OAP/FDP cycles. Set =0 for standalone FDP
  double tau_fdp_extra_standalone = 200e-6; // FDP extra duration for standalone runs
  int extra_frames_fdp_extra_standalone = 20; // Number of extra frames for FDP extra
  ...
  struct gk_mirror_ctx ctx = {
    ...
    .tau_fdp_extra_standalone = tau_fdp_extra_standalone,
    .extra_frames_fdp_extra_standalone = extra_frames_fdp_extra_standalone,
...
int main(int argc, char **argv)
{
  ...
  double total_phase_time = ctx.tau_oap + ctx.tau_fdp;
  if (ctx.num_cycles == 0 && ctx.tau_fdp_extra_standalone > 0.0) {
    // Standalone FDP mode
    ctx.t_end = ctx.tau_fdp_extra_standalone;
    ctx.num_frames = ctx.extra_frames_fdp_extra_standalone;
  } else {
    // Normal POA mode
  ...
  // Debug: Print initialization information
  gkyl_gyrokinetic_app_cout(app, stdout, "\n=== SIMULATION INITIALIZATION DEBUG INFO ===\n");
  gkyl_gyrokinetic_app_cout(app, stdout, "Simulation name: %s\n", app_inp.name);
  gkyl_gyrokinetic_app_cout(app, stdout, "Dimensions: cdim=%d, vdim=%d\n", app_inp.cdim, app_inp.vdim);
  gkyl_gyrokinetic_app_cout(app, stdout, "Configuration space: lower=[%g], upper=[%g]\n", app_inp.lower[0], app_inp.upper[0]);
  gkyl_gyrokinetic_app_cout(app, stdout, "Resolution: cells=[%d], poly_order=%d\n", app_inp.cells[0], app_inp.poly_order);
  gkyl_gyrokinetic_app_cout(app, stdout, "Velocity space (ion): lower=[%g, %g], upper=[%g, %g]\n", 
    ion.lower[0], ion.lower[1], ion.upper[0], ion.upper[1]);
  gkyl_gyrokinetic_app_cout(app, stdout, "Velocity resolution (ion): cells=[%d, %d]\n", ion.cells[0], ion.cells[1]);
  gkyl_gyrokinetic_app_cout(app, stdout, "Time: t_end=%g, num_frames=%d\n", ctx.t_end, ctx.num_frames);
  gkyl_gyrokinetic_app_cout(app, stdout, "POA parameters: num_cycles=%d, tau_oap=%g, tau_fdp=%g\n", 
    ctx.num_cycles, ctx.tau_oap, ctx.tau_fdp);
  gkyl_gyrokinetic_app_cout(app, stdout, "tau_fdp_extra=%g, extra_frames_fdp_extra=%d\n", 
    ctx.tau_fdp_extra, ctx.extra_frames_fdp_extra_standalone);
    gkyl_gyrokinetic_app_cout(app, stdout, "tau_fdp_extra_standalone=%g, frames_standalone_fdp=%d\n", 
    ctx.tau_fdp_extra_standalone, ctx.extra_frames_fdp_extra_standalone);
  gkyl_gyrokinetic_app_cout(app, stdout, "static_field_oap=%s, static_field_fdp=%s\n", 
    ctx.static_field_oap ? "true" : "false", ctx.static_field_fdp ? "true" : "false");
  gkyl_gyrokinetic_app_cout(app, stdout, "update_species_oap=%s, update_species_fdp=%s\n", 
    ctx.update_species_oap ? "true" : "false", ctx.update_species_fdp ? "true" : "false");
  gkyl_gyrokinetic_app_cout(app, stdout, "fdot_mult_type_oap=%d, fdot_mult_type_fdp=%d\n", 
    ctx.fdot_mult_type_oap, ctx.fdot_mult_type_fdp);
  gkyl_gyrokinetic_app_cout(app, stdout, "positivity_oap=%s, positivity_fdp=%s\n", 
    ctx.positivity_oap ? "true" : "false", ctx.positivity_fdp ? "true" : "false");\
  gkyl_gyrokinetic_app_cout(app, stdout, "oap_first=%s, frames_per_phase=%d\n", 
    ctx.oap_first ? "true" : "false", ctx.frames_per_phase);
  gkyl_gyrokinetic_app_cout(app, stdout, "Initial conditions: file_name=%s\n", ion.init_from_file.file_name);
  gkyl_gyrokinetic_app_cout(app, stdout, "Frames per phase: %d\n", ctx.frames_per_phase);
  gkyl_gyrokinetic_app_cout(app, stdout, "Physics: B_p=%g T, n0=%g m^-3, Te0=%g eV, Ti0=%g eV\n", 
    ctx.B_p, ctx.n0, ctx.Te0/ctx.qi, ctx.Ti0/ctx.qi);
  gkyl_gyrokinetic_app_cout(app, stdout, "Collision: nuIon=%g, nuElc=%g\n", ctx.nuIon, ctx.nuElc);
  gkyl_gyrokinetic_app_cout(app, stdout, "Geometry: psi_eval=%g, RatZeq0=%g\n", ctx.psi_eval, ctx.RatZeq0);
  gkyl_gyrokinetic_app_cout(app, stdout, "Parallelism: use_gpu=%d, cuts=[%d], use_mpi=%d\n", 
    app_args.use_gpu, app_args.cuts[0], app_args.use_mpi);
  gkyl_gyrokinetic_app_cout(app, stdout, "Enforce positivity: %s\n", app_inp.enforce_positivity ? "true" : "false");
  gkyl_gyrokinetic_app_cout(app, stdout, "Basis type: %d\n", app_args.basis_type);
  gkyl_gyrokinetic_app_cout(app, stdout, "============================================\n\n");
  ...
  bool is_standalone_fdp = (ctx.num_cycles == 0 && ctx.tau_fdp_extra_standalone > 0.0);
  ...
  if (app_args.is_restart) {
    ...
    if (!is_standalone_fdp) {
      // Only determine POA restart phase info if we're in POA mode
      ...
      gkyl_gyrokinetic_app_cout(app, stdout, "Restarting standalone FDP simulation\n");
    }
  }
  else {
  ...
  if (app_args.is_restart) {
    tfinal = t_curr;
    if (is_standalone_fdp) {
      // For standalone FDP, use current time and frame
      frame_end_phase = frame_curr;
    } else {
      // For POA restart, use restart info
      frame_end_phase = restart_info.phase_start_frame;
    }
  }
  ...
  // Skip to final FDP if: 1) standalone FDP mode, or 2) restarting in final FDP phase of POA
  bool skip_to_final_fdp = is_standalone_fdp || (app_args.is_restart && restart_info.is_final_fdp);
  ...
  if (skip_to_final_fdp) {
    ...
    if (is_standalone_fdp) {
      // Running standalone FDP phase
      gkyl_gyrokinetic_app_cout(app, stdout, "Running standalone FDP phase with extra frames\n");
      simulation_success = run_simulation_phase("FDP_EXTRA", ctx.tau_fdp_extra_standalone, fdp_extra_params, 
                                               ctx.extra_frames_fdp_extra_standalone, &tfinal, &frame_end_phase, app, &ctx, 
                                               &app_args, &t_curr, my_rank);
    } else {
      // Restarting in the final FDP phase of POA
