...
output_diagnostics(struct gk_mirror_ctx ctx, void *app_inp, struct gkyl_app_args app_args)
{
  ...
  if (my_rank == 0) {
    ...
    printf("Nonuniform mapping type = %d\n", app->geometry.position_map_info.id);
    printf("Nonuniform maximum slope at max B = %g\n", app->geometry.position_map_info.maximum_slope_at_max_B);
    printf("Nonuniform maximum slope at min B = %g\n", app->geometry.position_map_info.maximum_slope_at_min_B);
...
struct gkyl_efit_inp efit_inp = {
  ...
  struct gkyl_mirror_geo_grid_inp grid_inp = {
    ...
    .use_cubics = true, // use P3 C1 tracing for field line
  };
