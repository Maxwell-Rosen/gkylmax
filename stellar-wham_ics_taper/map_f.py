...
def remap_f(cql,ispec,ipsi,il,vbndry1,sin02thbndry1,vbndry2,vp2,zbnce,zbnce2,itype00):
    ...
    #initialize local scalar values
    zz0 = z[il] - z[0]
    BdB0_loc = BdB0[il]
    dPhi_loc = dPhi[il]
    vp2_loc = vp2[il]
    ...
    for iloc in range(iy):
        ...
        if (vp2_loc >= 0) and (zz0 <= zbnce2):
            ...
            uloc_z[jloc] = -np.sqrt(abs(vp2_loc))/v_norm #modified for coupling fix
            floc = 0.0
            floc_rz[jloc,iloc] = floc
        ...
        for jo in range(1,jx): 
            v02 = u0[jo]**2
            uloc2 = v02+vp2_loc
            if ((uloc2<0.0) or ((u0[jo]<=vbndry2) and (zz0>zbnce2))):
                jloc = jo
                uloc_z[jloc] = -np.sqrt(abs(uloc2))/v_norm #modified for coupling fix
                floc = 0.0
                floc_rz[jloc,iloc]=floc
                ...
                if sinn02>1.0:
                    ...
                    io = np.absolute(y[0:iyh]-theta0).argmin() #i think this should be ok for now 
