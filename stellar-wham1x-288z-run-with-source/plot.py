...
if plot_stitched_integrated_moms:
  ...
  ax[0,0].plot(continued_time, dM0_ion)
  ax[0,0].plot(orig_time, orig_dM0_ion)
  ax[0,0].set_xlim(0.5e-3, continued_time[-1])
  ax[0,0].set_ylim(-3e20, 3e20)
  ax[0,0].set_title('dM0/dt')
  ax[0,0].set_xlabel('Time')
  ax[0,0].set_ylabel('Derivative of integrated M0')

  ax[0,1].plot(continued_time, dM1_ion)
  ax[0,1].plot(orig_time, orig_dM1_ion)
  ax[0,1].set_xlim(0.5e-3, continued_time[-1])
  ax[0,1].set_title('dM1/dt')
  ax[0,1].set_xlabel('Time')
  ax[0,1].set_ylabel('Derivative of integrated M1')

  ax[1,0].plot(continued_time, dM2par_ion)
  ax[1,0].plot(orig_time, orig_dM2par_ion)
  ax[1,0].set_xlim(0.5e-3, continued_time[-1])
  ax[1,0].set_ylim(-1e32, 1e32)
  ax[1,0].set_title('dM2par/dt')
  ax[1,0].set_xlabel('Time')
  ax[1,0].set_ylabel('Derivative of integrated M2 parallel')

  ax[1,1].plot(continued_time, dM2perp_ion)
  ax[1,1].plot(orig_time, orig_dM2perp_ion)
  ax[1,1].set_xlim(0.5e-3, continued_time[-1])
  ax[1,1].set_ylim(-5e32, 5e32)
  ...
  X = np.column_stack((density, temp, temp_perp))
  ...
  norm = np.sqrt(dM0dt**2 + dM2pardt**2 + dM2perpdt**2)

  print(norm)

  # Fit a quadratic model for ||Y|| ~ (density, temp_par, temp_perp)

  poly = PolynomialFeatures(degree=2)
  X_poly = poly.fit_transform(X)
  model = LinearRegression().fit(X_poly, norm)

  # Define objective function
  def objective(x):
      model_poly = poly.transform(x.reshape(1, -1))
      return model.predict(model_poly)[0]

  # Use scipy.optimize to minimize ||Y||
  initial_guess = X[-1,:]  # Start at the average point
  res = minimize(objective, initial_guess, method='Nelder-Mead')

  # Suggested next test point
  suggested_next_point = res.x

  print(f"Suggested next point: density = {suggested_next_point[0]:.3f}, "
        f"temp_par = {suggested_next_point[1]:.3f}, "
        f"temp_perp = {suggested_next_point[2]:.3f}")
