...
class BeamSourceComparison:
  ...
  def __init__(self, 
         ...
         gamma_midmap=7.201e+01,
         T_beam_eV_spatial=200,
         T_beam_eV_midmap=32771.0,
  ...
  def optimize_midplane_map_to_match_bounce_avg(self, z_coords, vpar_coords, mu_coords,
    ...
    bounds = [
      ...
      (0.000001 * self.gamma_midmap, 1000.0 * self.gamma_midmap),
      (0.1 * T_beam_init_eV, 10000000.0 * T_beam_init_eV),
    ]
    ...
    def objective(params):
      ...
      temp_obj = BeamSourceComparison(
        ...
        E_beam_eV_midmap=0.0, # Makes this a Maxwellian
      ...
      M0_residual = ((M0_midmap_total - M0_bnc_avg_total) / M0_bnc_avg_total)**2
      ...
      power_residual = ((power_midmap_total - power_bnc_avg_total) / power_bnc_avg_total)**2
      
      total_residual = M0_residual + power_residual
  ...
  def plot_z_slice(self, z_cut=0.0, n_vpar=100, n_mu=100, n_workers=None):
    ...
    im1 = ax1.pcolormesh(vpar_plot, mu_plot, orbt_avg, shading='auto', cmap='inferno', vmin=vmin, vmax=vmax, rasterized=True)
    ...
    im2 = ax2.pcolormesh(vpar_plot, mu_plot, midmap, shading='auto', cmap='inferno', vmin=vmin, vmax=vmax, rasterized=True)
    ...
    im3 = ax3.pcolormesh(vpar_plot, mu_plot, diff, shading='auto', cmap='RdBu_r', 
                         vmin=-diff_max, vmax=diff_max, rasterized=True)
...
if __name__ == "__main__":
  ...
  bsc_optimized.compute_M0_fluxes(z_coords, vpar_coords, mu_coords)
  bsc_optimized.compute_power(z_coords, vpar_coords, mu_coords)
  
  # mu_val = 4e-15
  # fig3 = bsc_optimized.plot_vpar_cut(mu_val, z_cut=0.0)
  # fig3.savefig('source_comparison_vpar_cut.pdf')

  # # bsc_optimized.compute_power(z_coords, vpar_coords, mu_coords)
  # fig4= bsc_optimized.plot_z_slice(z_cut=0.0, n_vpar=100, n_mu=100)
  # fig4.savefig('source_comparison_slices.pdf')
  # # fig3.suptitle('After optimization')

  # fig5 = bsc_optimized.plot_comparison_2d(z_coords, vpar_coords, mu_val)
  # fig5.savefig('source_comparison_2d.pdf')

  # plt.show()


  #   E_beam_midmap: 25000.0 eV -> 25757.9 eV
  # T_beam_midmap: 200.0 eV -> 221.2 eV
  # gamma_midmap: 1.110e+03 -> 9.876e+02