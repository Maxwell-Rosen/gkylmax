...
first_job_id=$(sbatch jobscript-gkyl-perlmutter | awk '{print $4}')
job_ids+=($first_job_id)

# Submit subsequent jobs with dependency on the previous job
for i in {2..4} # Adjust the range as needed for the number of jobs
do
...
# Print all job IDs
echo "Submitted jobs with IDs: ${job_ids[@]}"1