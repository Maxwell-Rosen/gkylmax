...
dataDir = '/home/mr1884/scratch/Link to scratch_traverse/gkylmax/traverse-wham1x-compare_positivity/outputs/'
...
plot_distf_at_z    = 1
z_loctions = np.array([0.92, 0.98, 1.04])#np.array([0, 0.3, 0.98, 2.4])
...
def process_frame(frameNum):
  ...
  if plot_moments:
    ...
    # Plot the density
    ax[0,0].plot(dataOut_unif_mapc2p[:,0], dataOut_unif[:,0],'r', label='No correction')
    ax[0,0].plot(dataOut_nonunif_mapc2p[:,0], dataOut_nonunif[:,0],'b--', label='Ad-hoc positivity correction')
