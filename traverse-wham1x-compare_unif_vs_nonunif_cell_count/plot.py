...
dataDir = '/home/mr1884/scratch/Link to scratch_traverse/gkylmax/traverse-wham1x-compare_unif_vs_nonunif_cell_count/outputs/'
...
def process_frame(frameNum):
  ...
  def load_mapped_data(dataName):
    ...
    densityFileName_coarse = str(dataDir+coarseFile + str(dataName) + str(frameNum) + '.gkyl')
    pgData_coarse = pg.GData(densityFileName_coarse)
    pgInterp_coarse = pg.GInterpModal(pgData_coarse, polyOrder, 'ms')
    x_coarse, dataOut_coarse = pgInterp_coarse.interpolate()
    ...
    coarse_mapc2p_filename = str(dataDir+coarseFile+'-mapc2p.gkyl')
    pgData_coarse_mapc2p = pg.GData(coarse_mapc2p_filename)
    pgInterp_coarse_mapc2p = pg.GInterpModal(pgData_coarse_mapc2p, polyOrder, 'ms')
    x_coarse_mapc2p, dataOut_coarse_mapc2p = pgInterp_coarse_mapc2p.interpolate(2)
    

    return dataOut_unif, dataOut_unif_mapc2p, dataOut_nonunif, dataOut_nonunif_mapc2p, dataOut_reduced, dataOut_reduced_mapc2p, dataOut_coarse, dataOut_coarse_mapc2p
  ...
  if plot_moments:
    dataOut_unif, dataOut_unif_mapc2p, dataOut_nonunif, dataOut_nonunif_mapc2p, dataOut_reduced, dataOut_reduced_mapc2p, dataOut_coarse, dataOut_coarse_mapc2p = load_mapped_data('-ion_M0_')
    ...
    # Plot phi
    dataOut_unif, dataOut_unif_mapc2p, dataOut_nonunif, dataOut_nonunif_mapc2p, dataOut_reduced, dataOut_reduced_mapc2p, dataOut_coarse, dataOut_coarse_mapc2p = load_mapped_data('-field_')
    ...
    # Plot uPar
    M0_unif, M0_map, M0_nonunif, M0_nonunif_map, M0_reduced, M0_reduced_map, M0_coarse, M0_coarse_map = load_mapped_data('-ion_M0_')
    M1_unif, M1_map, M1_nonunif, M1_nonunif_map, M1_reduced, M1_reduced_map, M1_coarse, M1_coarse_map = load_mapped_data('-ion_M1_')

    upar_unif = M1_unif[:,0]/M0_unif[:,0]
    upar_nonunif = M1_nonunif[:,0]/M0_nonunif[:,0]
    upar_reduced = M1_reduced[:,0]/M0_reduced[:,0]
    upar_coarse = M1_coarse[:,0]/M0_coarse[:,0]
    ...
    ax[1,0].plot(M0_coarse_map[:,0], upar_coarse / c_s,'k-.')
    ...
    M2perp_unif, M2perp_map, M2perp_nonunif, M2perp_nonunif_map, M2perp_reduced, M2perp_reduced_map, M2perp_coarse, M2perp_coarse_map = load_mapped_data('-ion_M2perp_')
    ...
    tPerp_coarse = M2perp_coarse[:,0]/M0_coarse[:,0] * mi / eV
    ...
    ax[1,1].plot(M0_coarse_map[:,0], tPerp_coarse,'k-.')
    ...
    M2par_unif, M2par_map, M2par_nonunif, M2par_nonunif_map, M2par_reduced, M2par_reduced_map, M2par_coarse, M2par_coarse_map = load_mapped_data('-ion_M2par_')
