...
eval_f_ion_source(double t, const double *GKYL_RESTRICT xn, double *GKYL_RESTRICT fout, void *ctx)
{
  ...
  double E_beam = 25000.0 * GKYL_ELEMENTARY_CHARGE; // Beam intM2 = 7.682495e+32
...
create_ctx(void)
{
  ...
  double tau_oap = 0.2;  // Duration of each phase.
  double tau_fdp = 15e-6;
  double tau_fdp_extra = 10*15e-6;
  ...
  int num_frames_fdp_extra = 10*5;  // Frames for the extra FDP phase
  ...
  ctx.psi_eval = psi_RZ(ctx.RatZeq0, 0.0, &ctx)* 1. / 2.;
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_mirror_geo_grid_inp grid_inp = {
    .filename_psi = "/global/homes/m/mhrosen/scratch/gkylmax/generate_efit/lorentzian_R10.geqdsk_psi.gkyl", // psi file to use
